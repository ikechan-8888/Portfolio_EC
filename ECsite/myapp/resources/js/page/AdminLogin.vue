<template>
  <div>
    ...
    <div class="container--small">
      <ul class="tab">
        <li class="tab__item tab__item--active" v-if="!isLogin">
          ログイン
        </li>
        <li class="tab__item tab__item--active" v-else-if="isLogin">
          会員登録
        </li>
      </ul>
      <div class="panel form_panel" v-if="!isLogin">
      ...
      <div class="panel form_panel" v-else-if="isLogin">
...
export default{
  ...
  methods: {
    ...
    async register () {
      ...
      await this.$store.dispatch('auth/adminregister', this.registerForm);
      this.loading = false;
      //statuscode=200だったら
      if(this.apiStatus){
        // トップページに移動する(resourses/js/router.jsのルートに/が追加される)
        this.$router.push('/adminTopPage');
      }
    },
    async login(){
      this.loading = true;
      await this.$store.dispatch('auth/adminlogin', this.loginForm);
      this.loading = false;
      //statuscode=200だったら
      if(this.apiStatus){
        this.$router.push('/adminTopPage');
    ...
    clearError(){
      this.$store.commit('auth/setAdminLoginErrorMessages', null);
      this.$store.commit('auth/setAdminRegisterErrorMessages', null);
    }
  },
  computed: {
    apiStatus(){
    ...
    loginErrors(){
      return this.$store.state.auth.adminLoginErrorMessages;
    },
    registerErrors(){
      return this.$store.state.auth.adminRegisterErrorMessages;
    }
  },
  created(){
    this.clearError();
