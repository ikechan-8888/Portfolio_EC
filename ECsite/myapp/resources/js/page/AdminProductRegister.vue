<template>
  <div>
    ...
    <AdminProductCheck class="overlay" v-show="checkImage" :key="registerForm.id" :item="registerForm" @close="onclose" />
    <div class="container--registerform">
      <div class="form_top">
        <h1 class="form_name">新規商品登録(再読み込みすると入力内容がクリアされしまいます)</h1>
...
<!-- 商品画像　-->
          <div class="form_block_admin">
            <div class="form_block_admin_block">
              <label for="register-img" class="form_label">商品画像</label><span class="form_require">必須(jpeg,jpg,png形式で選択してください)</span>
            </div>
            <div v-if="registerErrors">
...
<!-- 商品の在庫数 -->
          ...
          <div class="admin_form__buttons">
            ...
            <button type="submit" v-show="button === 2" class="button adminregister_form_button">登録する</button>
          </div>
        </form>
      </div>
    </div>
...
import AdminProductCheck from '../components/AdminProductCheck.vue'
import { OK } from '../util'
...
export default{
  ...
  methods: {
    async register(){
      ...
      const productResponse = await axios.post('/api/productregister', data);
      if(productResponse.status !== OK){
          this.$store.commit('error/setCode', productResponse.status);
          return false;
      }
      this.loading = false;
      alert('登録した商品を販売開始にするにはトップページから対象商品の販売開始ボタンをクリックして下さい');
